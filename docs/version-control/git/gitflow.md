...
| Ветка       | Назначение                         |
| ----------- | ---------------------------------- |
| `main`      | Продакшн-готовый код, теги релизов |
...
##### Вариант 1: Через git flow (без код-ревью)

```bash
git flow feature finish <номер-задачи>
```

##### Вариант 2: Через Merge Request (с код-ревью)
...
Используется когда нужно срочно исправить баг в продакшене.
...
| GitHub Flow | Небольшие команды, частые деплои      |
