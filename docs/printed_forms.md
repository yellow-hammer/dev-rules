...
1. **Размещение в обработке**:
   - Печатная форма размещается в _отдельной обработке_, если:
     ...
     - Необходимо ограничить доступ к печатной форме с помощью _ролей_.
     - Если реализация печатной формы выполняется _подрядчиком_, и требуется изоляция его кода для предотвращения влияния на основную конфигурацию.
...
- Если дорабатывается конкретный объект метаданных, методы **`Печать`** и **`ДобавитьКомандыПечати`** перенаправляются из модуля менеджера на общий модуль, созданный для этого объекта.
  - Например, для доработки документа _"ПриемНаРаботу"_ перенаправление должно происходить на общий модуль **`прфПриемНаРаботу`**.

- В общем модуле методы размещаются в области _СлужебныйПрограммныйИнтерфейс_ и оформляются по аналогии с типовым кодом.
