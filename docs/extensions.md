...
## Приоритет стратегий переопределения

При выборе типа расширяющего метода придерживаться следующего приоритета:
...
Использование `Вместо` допускается только при невозможности решить задачу через более безопасные стратегии.
...
- Если отчёт или печатная форма типовая, то зависит от сложности отчёта (печатной формы) и степени доработки. Какие-то сложные регламентированные формы/отчеты дорабатывать напрямую, через расширение.
- Если отчёт/форма простая и объем доработок большой, то имеет смысл делать свою копию объекта целиком.
