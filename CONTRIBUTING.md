...
3. **Разработка**
   ...
   - Проверьте качество документации:
     ...
     npm run lint:md:fix    # автоисправление проблем markdown
...
# Lighthouse (требует запущенный сервер)
npm run lighthouse:mobile  # проверка мобильной версии
npm run lighthouse:desktop # проверка десктопной версии
```

### VS Code Tasks

Все основные команды также доступны через VS Code Tasks (Ctrl+Shift+P → "Tasks: Run Task"):
